...
from lifelike.networks.legged_robot.sepmc_net.sepmc_net_data import \
    SEPMCInputs, SEPMCOutputs, SEPMCTrainableVariables, \
...
def sepmc_inputs_placeholder(nc: SEPMCConfig):
...
def hlc_encoder(prop, hlc_usr_cmd, mlc_usr_cmd, hs, m, nc):
    ...
    return hlc_head, hs_new


def mlc_usr_cmd_encoder(usr_cmd, nc):
    usr_cmd_all_embeds = []
    keys = list(usr_cmd.keys())
    percept_2d_embed = percept_2d_encoder(usr_cmd['percept_2d'])
    usr_cmd_all_embeds.append(percept_2d_embed)
    keys.pop(keys.index('percept_2d'))
    if nc.take_percept_1d:
        percept_1d_embed = percept_1d_encoder(usr_cmd['percept_1d'], nc)
        usr_cmd_all_embeds.append(percept_1d_embed)
    keys.pop(keys.index('percept_1d'))
    percept_front_embed = percept_2d_encoder(usr_cmd['percept_front'], scope='percept_front')
    usr_cmd_all_embeds.append(percept_front_embed)
    keys.pop(keys.index('percept_front'))
    if len(keys) > 0:
        # vec feat
...
def sepmc_net(inputs: SEPMCInputs,
    ...
    with tf.compat.v1.variable_scope(scope, default_name='model') as sc:
        ...
        mlc_usr_cmd = OrderedDict({
            'percept_2d': inputs.X['percept_2d'],
            'percept_1d': inputs.X['percept_1d'],
            'percept_front': inputs.X['percept_front'],
        })
        hlc_usr_cmd_pi = OrderedDict({
            'percept_vec': inputs.X['percept_vec'],
            'oppo_info': inputs.X['oppo_info'],
            'flag_info': inputs.X['flag_info'],
            'with_flag': inputs.X['with_flag'],
        })
        hlc_usr_cmd_vf = OrderedDict({
            'percept_vec': inputs.X['percept_vec'],
            'oppo_info_cheat': inputs.X['oppo_info_cheat'],
            'flag_info_cheat': inputs.X['flag_info_cheat'],
            'with_flag': inputs.X['with_flag'],
        })
        ...
        hs_z = inputs.S[:, nc.hs_len * 2:nc.hs_len * 3]
        hs_hlc = inputs.S[:, nc.hs_len * 3:]
        ...
        with tf.compat.v1.variable_scope('expert_pi'):
            if nc.use_self_fed_heads:
                ...
                hlc_curr = hlc_head.sam
                if nc.outer_control_spd:
                    mlc_usr_cmd.update({
                        'target_info': tf.concat(
                            [tf.cos(hlc_curr), tf.sin(hlc_curr), inputs.X['control_spd']], axis=-1)
                    })
                else:
                    mlc_usr_cmd.update({
                ...
                self_fed_heads = tp_utils.pack_sequence_as_structure_like_gym_space(
                    nc.ac_space, [hlc_head, z_head, llc_head])
            else:
                hlc_head, hs_hlc_new = hlc_encoder(
                    prop_rms_extended, hlc_usr_cmd_pi, mlc_usr_cmd, hs_hlc, inputs.M, nc)
                flag = inputs.A['A_HLC'] is not None
                assert flag, ('creating outer_fed_heads, '
                              'but outer fed head A_HLC is None ...')
                hlc_curr = inputs.A['A_HLC']
                if nc.outer_control_spd:
                    mlc_usr_cmd.update({
                        'target_info': tf.concat(
                            [tf.cos(hlc_curr), tf.sin(hlc_curr), inputs.X['control_spd']], axis=-1)
                ...
                with tf.compat.v1.variable_scope(nc.llc_param_type):
                    ...
                    if nc.llc_light:
                        ...
                        llc_head, hs_pi_new = llc(prop_rms, z_curr, hs_pi, inputs, nc)
                outer_fed_heads = tp_utils.pack_sequence_as_structure_like_gym_space(
                    nc.ac_space, [hlc_head, z_head, llc_head])

        hs_new = tf.concat([hs_vf_new, hs_pi_new, hs_z_new, hs_hlc_new], axis=-1)
...
# APIs
net_build_fun = sepmc_net
net_config_cls = SEPMCConfig
net_inputs_placeholders_fun = sepmc_inputs_placeholder
